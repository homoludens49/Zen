  ...
  AutoOrder.find({}).then((orders) => {
    ...
    const lastMonthOrders = orders.filter(
      (order) =>
        ...
        order.orderDate.getMonth()-1 === new Date().getMonth()-1 &&
        order.orderDate.getFullYear() === new Date().getFullYear()
    );
    ...
    const sumOrders = (orders) => {
      ...
      for (let i = 0; i < orders.length; i++) {
        ...
        for (let j = 0; j < orders[i].order.length; j++) {
          ...
          const orderedProducts  = products.filter( product => product.sku === orders[i].order[j].sku);
          //get costOfGoodsSold from orderedProducts buyprice and quantity
          if(orderedProducts.length > 0){
            costOfGoodsSold += parseFloat(orderedProducts[0].buyprice) * parseFloat(orders[i].order[j].quantity);
        ...
        totalExpenses =
          ...
          makecommerceFee +
          paypalFee;
        totalNet = sum + shipping - totalExpenses -costOfGoodsSold;
      }
      return {
        sum: sum,
